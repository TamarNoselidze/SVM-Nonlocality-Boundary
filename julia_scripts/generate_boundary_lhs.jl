    ...
    while count < n_target
        ...
        if ppt(rho, [2, 2], 1) < -1e-6
            ...
            if v_lower < 1-1e-5
                ...
                # sample eta between v_lower and 1, skewed toward v_lower
                u = rand()
                k = 3  # controls skew s.t. higher = more boundary-concentrated
                eta = v_lower + (1 - v_lower) * u^k

                rho_B = ptrace(rho, [2, 2], 1)
                rho_lhs = eta * rho + (1 - eta) * kron(I_half, rho_B)

                count += 1
                valid_states[count] = rho_lhs
...
h5open("states_lhs_boundary_$(batch_id).h5", "w") do file
    write(file, "rho", states_3d)
end

h5open("labels_lhs_boundary_$(batch_id).h5", "w") do file
